...
1. **The receptor is a model, and its conformational state is not established.** The reference oligomer is constructed under a published topology prior, not from atomic coordinates. The DARR set in Fusco et al. (2017) is too sparse to determine the geometry uniquely; the relaxed structure is one of many compatible with the published topology. The ensemble check across 11 topologies (§3) shows only that the *activity* score is robust to the starting build — not that this conformation is metastable. Unbiased MD shows it is not: the built geometry is vacated within tens of ns and relaxes ~6–9 Å away into a different basin that still scores toxic (EXPERIMENTS.md E5); and across diverse independent starts there is no single attractor — the relaxed states split by β-sheet register into distinct, history-dependent basins that do not simplify with oligomer size (E6, E7). All three channels depend on the receptor shape, so every ranking is conditional on a single, hand-picked conformation the system does not actually populate. An occupancy Markov state model (E8) now estimates where the trimer's mass actually sits: each register concentrates in one or two dominant basins 4–8 Å from that pose, and every populated basin still scores toxic (occupancy-weighted activity ≈ 11–13 vs a harmless floor of 3.7) — so a populated, still-toxic target ensemble does exist, and the channels can be re-grounded on it (the per-register occupancy-weighted medoid ensemble) rather than the single pose. The rankings in this release are *not yet* re-grounded, and which register is biologically real still needs an experimental structure.
...
1. **The target is a model, and it does not hold the exact shape we built.** No one has experimentally captured the toxic oligomer; we built it from published constraints. We checked it against 11 alternative builds, but those checks only test the toxicity *score*, not whether the structure keeps its shape on its own — and under unbiased simulation it slides ~6–9 Å off the built shape into a nearby, still-toxic-looking basin. Worse for the "one target" idea: different starting builds settle into *different* basins depending on sheet direction (and this doesn't tidy up at other oligomer sizes), so which shapes the oligomer actually occupies — the shapes every ranking hangs on — is still unknown.
