...
- **Run longer-timescale MD around docked poses.** The pilot in `screen/md_stage3.py` was inconclusive at 50 ps. ≥10 ns runs would let stabilising binders show positive Δact through receptor rearrangement, partly removing the §8.3 sign-bound. The shape-stability / dwell-time channel (`screen/dwell_time.py`, [#14](https://github.com/xag/asyn-oligomer-screen/issues/14)) now realises this as a multi-replica ensemble-shift test, designed to run as independent per-GPU chunks; the harness is in-repo and the apo side is validated on a basic GPU (see §9).
