// Build the wet-lab handoff package (docs/HANDOFF.md).
//
...
// `assay`   the orthogonal-mechanism readout this molecule needs
...
function protectiveSection() {
  ...
  lines.push('');
  lines.push('⭐ = **recommended first-pass trio** (best supply + lowest regulatory friction): DHEA, piperine, urolithin A. All OTC and cheap.');
  lines.push('');
  PROTECTIVE.forEach((id, i) => {
...
- **Orthogonal mechanism is required.** A direct-binding-only assay cannot confirm a destabiliser whose mechanism is conformational or autophagic. Use readouts that detect aggregation/oligomer-state change (ThT kinetics, DLS), not just binding.
...
- **Polyphenol bias is partly framework-coupled** — the novel non-polyphenol hits carry more signal per rank than the polyphenol confirmatory band.
- **Scores are ordinal, not magnitude-interpretable** — read the rank order, not the absolute Δact.
- **Δact is sign-bound by static-pose features** — the screen currently sees destabilisers only.

## Pending: anti-target axis

The "what to avoid" half here is the **covalent** axis only. The symmetric **shape-stabiliser** anti-target axis (ligands that stabilise toxic shapes) is blocked on the multi-replica short-MD dwell-time channel and its post-processing. When that lands, this package gains an anti-target flag column. Until then, the covalent reactive metabolites are the only confirmed-mechanism "avoid" entries.
